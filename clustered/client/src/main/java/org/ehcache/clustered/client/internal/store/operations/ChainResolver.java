...
public class ChainResolver<K, V> {
  ...
  public ResolvedChain<K, V> resolve(Chain chain, K key, long now) {
    ...
    if(compacted) {
      ...
      return new ResolvedChain.Impl<K, V>(chainBuilder.build(), key, result, true, expirationTime);
    } else {
      return new ResolvedChain.Impl<K, V>(chain, key, result, false, expirationTime);
    }
  }
}
